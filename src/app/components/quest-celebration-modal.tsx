...
export function QuestCelebrationModal({
  ...
  return (
    <>
      {/* Confetti renders outside the modal stack so it covers the whole viewport */}
      {showConfetti && (
        <Confetti
      ...
      <ModalBackdrop isOpen={isOpen} onClose={onClose}>
        <motion.div
          className="modal-panel border-2 border-cyan-500/50 w-[90vw] md:w-full max-w-lg overflow-hidden"
        ...
        >
          ...
          <div className="relative px-6 py-8 bg-gradient-to-br from-cyan-600/20 via-blue-600/20 to-purple-600/20">
            ...
            >
              {targetType === 'role' ? <Target className="w-5 h-5" /> : <Sparkles className="w-5 h-5" />}
              <p className="text-lg">{targetType === 'role' ? 'Target Role' : 'Target Skill'}</p>
            </motion.div>
          </div>
          ...
          <div className="px-6 py-6">
            ...
            >
              <p className="text-center text-gray-300 mb-2">You&apos;ve completed all tasks for:</p>
            ...
            >
              {targetType === 'skill' && onUpdateProficiency && (
                <button
                  onClick={() => { onUpdateProficiency(); onClose(); }}
              ...
              <button
                onClick={onClose}
                className="btn-ghost w-full py-3 border border-slate-700"
              >
